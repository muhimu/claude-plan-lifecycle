---
description: Restack a stacked-PR chain after squash-merges to the default branch — discovers the stack from GitHub, cascade-rebases surviving branches, verifies the tip, then (after one confirmation) force-with-lease pushes and retargets the new bottom PR.
...
stacked mode) squash-merged to the default branch. Rebases every surviving
branch onto the new default-branch head (cascade), verifies the tip, and —
after a single confirmation — force-with-lease pushes the rebased branches
and retargets the new bottom PR to the default branch.
...
3. **Parse the ordering.** Read the `## Stack` table from that PR's body:
   ...
   previous PR's head; the bottom's base is `$DEFAULT_BRANCH`). If neither yields a
...
6. **Print the plan of record** before touching anything:
   ...
   Restack:   B[1] ← origin/$DEFAULT_BRANCH
              B[2] ← B[1]
              …
   ```
...
2. **Rebase:**
   ...
   ```
   where for i = 1: `NEW_BASE=origin/$DEFAULT_BRANCH`, `OLD_BASE=$OLD_BASE_SHA`; for
...
3. **On conflict, assess before resolving.** Inspect each conflicted file
   ...
   - **Mechanical** — the conflict exists only because the squash commit on
     the default branch already contains the merged slice's version of these lines (the
     incoming side and the new base agree in content, only history differs),
     or a stack commit is now fully contained in the default branch (`git diff` after
...
with `origin/$DEFAULT_BRANCH` equals its head and `git rev-list --count
...
2. **Update `## Stack` tables.** For each surviving PR, edit ONLY its
   ...
   `$DEFAULT_BRANCH`. Same sanction boundary as `/execute-plan` S4 — bodies of this
   stack's PRs only.
3. **Report:**

   ```
   Restacked <S> branch(es) onto origin/$DEFAULT_BRANCH:
